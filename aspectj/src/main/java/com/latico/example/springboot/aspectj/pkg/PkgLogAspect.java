package com.latico.example.springboot.aspectj.pkg;
...
@Component
@Aspect
public class PkgLogAspect {

    private static final Logger logger = LoggerFactory.getLogger(PkgLogAspect.class);
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    /**
     * 利用匹配包下面的方法
     */
    private final String POINT_CUT_PACKAGE = "execution(public * com.latico.example.springboot.aspectj.pkg.*.*(..))";
    ...
    public void doAfterThrowingAdvice(JoinPoint joinPoint, Throwable exception) {
        ...
        if (exception instanceof NullPointerException) {
            logger.info("PKG 发生了空指针异常!!!!!");
        }
    }

    @After(value = POINT_CUT_PACKAGE)
    public void doAfterAdvice(JoinPoint joinPoint) {
        logger.info("PKG 后置通知执行了!");
    }
    ...
    public Object doAroundAdvice(ProceedingJoinPoint proceedingJoinPoint) {
        logger.info("PKG @Around环绕通知：" + proceedingJoinPoint.getSignature().toString());
        Object obj = null;
        try {
        ...
        logger.info("PKG @Around环绕通知执行结束");
        return obj;
    }
}