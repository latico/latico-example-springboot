...
public class AnnoLogAspect {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AnnoLogAspect.class);
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    private final String POINT_CUT_ANNOCATION = "execution(public * com.latico.example.springboot.aspectj.*.*(..))";

    @Pointcut("@annotation(logAnnotation)")
    public void annoPointcut(LogAnnotation logAnnotation) {
    }

    @Before("annoPointcut(logAnnotation)")
    public void before(JoinPoint joinPoint, LogAnnotation logAnnotation) {
        logger.info("Anno @Before通知执行");
        //获取目标方法参数信息
     ...
     */
    @AfterReturning(value = "annoPointcut(logAnnotation)", returning = "result")
    public void doAfterReturningAdvice1(JoinPoint joinPoint, Object result, LogAnnotation logAnnotation) {
        logger.info("第一个后置返回通知的返回值：" + result);
    }

    @AfterReturning(value = "annoPointcut(logAnnotation)", returning = "result")
    public void doAfterReturningAdvice2(String result, LogAnnotation logAnnotation) {
     ...
     * @param exception
     */
    @AfterThrowing(value = "annoPointcut(logAnnotation)", throwing = "exception")
    public void doAfterThrowingAdvice(JoinPoint joinPoint, Throwable exception, LogAnnotation logAnnotation) {
        logger.info(joinPoint.getSignature().getName());
        if (exception instanceof NullPointerException) {
            logger.info("Anno 发生了空指针异常!!!!!");
        }
    }

    @After("annoPointcut(logAnnotation)")
    public void doAfterAdvice(JoinPoint joinPoint, LogAnnotation logAnnotation) {
        logger.info("后置通知执行了!");
    }
     ...
     */
    @Around("annoPointcut(logAnnotation)")
    public Object doAroundAdvice(ProceedingJoinPoint proceedingJoinPoint, LogAnnotation logAnnotation) {
        logger.info("Anno @Around环绕通知：" + proceedingJoinPoint.getSignature().toString());
        Object obj = null;
        try {
        ...
        logger.info("Anno @Around环绕通知执行结束");
        return obj;
    }
}